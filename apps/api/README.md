...
4. **Sub-Agent 1 (Smart Connector)** fetches raw rows from the scanner, persists
   them verbatim into `raw_findings`, then for each row calls Claude to produce
   a canonical Issue and inserts it into `issues`.
5. **Sub-Agent 2 (Enrichment Specialist)** queries EPSS, CISA KEV, and NVD for
   every CVE, then for each Issue calls Claude to compute a `derived_risk`
...
### Anthropic `tool_use` for all structured output
When we call Claude and need a structured response (a canonical Issue, a risk
decision), we use the `tool_use` feature with a Pydantic-derived JSON schema.
**Anthropic enforces the schema at the API level** — the response is guaranteed
...
└── app/
    ...
    └── agents/
        ...
        ├── llm.py            # Anthropic SDK client (lazy singleton)
...
Anthropic, NVD, optional Tenable). Real `.env` is gitignored.
...
- **`LLMNormalizedIssue`** — Sub-Agent 1's LLM output schema. Used as the `input_schema` for Anthropic `tool_use` so Claude's response is guaranteed-shaped. 13 fields (the canonical normalization output, fields 1–14 of the bigger Issue schema). `model_config = ConfigDict(extra="forbid")` rejects any unknown field from the LLM.
...
### `app/agents/llm.py`
Lazy singleton wrapper for the Anthropic client. Reads `ANTHROPIC_API_KEY`
from settings, returns a shared `Anthropic` client. Both Sub-Agent 1 and
Sub-Agent 2 call `get_client()` and reuse the same instance.

### `app/agents/master.py`
The Master Agent. Currently **deterministic Python** (not LLM-driven yet — the
LLM layer would land if we needed dynamic planning across many scanners).

Function: `run_master(run_id)`.
1. Marks `agent_runs.status = "running"`.
2. Emits trace `"Run started, planning"`.
3. Reads the run's `targets.scanners`. If `["all"]`, expands to every enabled
   tool in `connection_registry`.
4. **For each scanner**: looks up its row in `connection_registry`, emits a
   `DISPATCH` trace, calls `sub_agent_1.run_fetch(...)`, emits the
   `Received FETCH_DONE` trace. Per-scanner errors are caught and reported but
   don't kill other scanners' processing.
5. Emits `FETCH phase complete`.
6. Dispatches `ENRICH` to Sub-Agent 2 once. Calls `sub_agent_2.run_enrich(run_id)`.
7. Emits `Received ENRICH_DONE`.
8. Marks the run completed with full `summary` JSON (counts, EPSS hits, KEV hits, NVD hits).
9. Emits `SCAN_COMPLETE`.
...
**Step 3 — Normalize each row via Claude.**
...
- For each persisted raw row:
  - Builds a JSON message of `{source_scanner, raw_row, mapping_rules}`.
  - Calls Claude with `tool_use` and the `LLMNormalizedIssue` schema.
  - Anthropic returns a guaranteed-shaped `ToolUseBlock`. We construct a Pydantic
    model from `tool_block.input` (defense in depth).
...
- Tracks errors per-row. First 3 failures emit an ERROR trace with the actual exception.
...
**Step 4 — LLM decision per issue.**
- Helper `_llm_decide(prompt_row, issue, epss, nvd, in_kev)` builds a JSON
  payload of issue + enrichment data and calls Claude with the
  `LLMEnrichmentDecision` schema via `tool_use`.
- Claude returns: `derived_risk` (0–100), `risk_explanation`, `likelihood`,
...
| Service | Where | Auth |
|---|---|---|
| **Anthropic Claude** | `agents/llm.py` | `ANTHROPIC_API_KEY` |
...
- **Sub-Agent 1 (Smart Connector)** — talks to one scanner at a time, fetches raw findings, persists them, normalizes each into the canonical Issue shape via Claude.
- **Sub-Agent 2 (Enrichment Specialist)** — for every Issue, fetches EPSS / KEV / NVD data and asks Claude to produce a risk score with explanation + remediation.
...
- **`tool_use`** — Anthropic feature where the LLM is told "call this function with these typed arguments." API enforces the schema; we get structured data without parsing.
